...
func init() {
	...
	curdhost.PromptSelect = func(options []curdhost.PromptOption) (curdhost.PromptOption, error) {
		...
		selected, err := DynamicSelect(mapped)
		if err != nil {
	...
	curdhost.PersistSubStylePreference = func(style string) error {
		...
		if GlobalConfigPath != "" {
			if m, err := LoadConfigFromFile(GlobalConfigPath); err == nil {
				m["SubStyle"] = style
				return SaveConfigToFile(GlobalConfigPath, m)
			}
		}
		return nil
	}
	...
	curdhost.StoragePath = func() string {
		if cfg := GetGlobalConfig(); cfg != nil {
